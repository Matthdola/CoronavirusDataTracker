...
public class CoronavirusDataService {
	...
	public void fetchVirusData() throws IOException, InterruptedException{
		...
		for (CSVRecord record : records) {
			...
			locationStats.setLongitude(Double.parseDouble(record.get("Long_")));
			...
			locationStats.setConfirmed(Integer.parseInt(record.get("Confirmed")));
			locationStats.setDeaths(Integer.parseInt(record.get("Deaths")));
			
			// locationStats.setLatestTotalCases(Integer.parseInt(record.get(record.size() - 1)));
			int latestCases = 0;              // Integer.parseInt(record.get(record.size() - 1));
			int previousDaysCases = 0;        // Integer.parseInt(record.get(record.size() - 2));
			
			locationStats.setLatestTotalCases(latestCases);
			locationStats.setDiffFromPreviousDay(latestCases - previousDaysCases);
